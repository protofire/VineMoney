 ...
 */
contract DepositVineLpToken is VineOwnable {
    ...
    function checkDlpStatus(uint256 debt, address account) public view returns (bool) {
        ...
        IStdReference.ReferenceData memory data = lpOracle.getReferenceData('VINELP', 'USD');
        if(balanceOf[account] * data.rate / 1e18 * 1000 / debt > dlpThresold) return true;
        return false;
    }
}
